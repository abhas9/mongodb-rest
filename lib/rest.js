        ...
        if(req.body) {
            connection.connect(dbConnectionStr + '/' + req.params.db, function (err, db) {
                ...
                db.collection(req.params.collection, function(err, collection) {
                    ...
                    var postedJSON = Array.isArray(req.body) ? req.body[0] : req.body;
                    // Add metadata of uploaded files
                    if (req.files) {
                        var propertiesToPersist = ["size","path"]
                        for (var file in req.files) {
                            for (var i = 0 ; i < req.files[file].length ; i++) {
                                for (var fileProperty in req.files[file][i]) {
                                    if (propertiesToPersist.indexOf(fileProperty) < 0) {
                                        delete req.files[file][i][fileProperty];
                                    }
                                }
                            }
                            postedJSON[file] = req.files[file];
                        }
                    }
                    collection.insert(postedJSON, 
                        function(err, docs) {
                            if (err) {
